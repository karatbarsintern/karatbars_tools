...
use KaratbarsToolsLogManager\Domain\Site\SiteRepository;
...
class PhoneEmailListTaskAdditionalFieldProvider implements AdditionalFieldProviderInterface
{
    ...
    public function getAdditionalFields(
        ...
        if ($schedulerModule->CMD === 'add') {
            $taskInfo['phoneEmailListFile'] = 50;
        }

        if ($schedulerModule->CMD === 'edit') {
            $taskInfo['phoneEmailListFile'] = $task->getPhoneEmailListFile();
        }


        $additionalFields['phoneEmailListFile'] = [
            'code' => '<input type="number" class="form-control" name="tx_scheduler[phoneEmailListFile]" value="' . htmlspecialchars($taskInfo['phoneEmailListFile']) . '" />',
            'label' => 'LLL:EXT:karatbarstools/Resources/Private/Language/locallang.xlf:indexqueueworker_field_phoneEmailListFile',
            'cshKey' => '',
            'cshLabel' => ''
        ];
    ...
    public function validateAdditionalFields(
        ...
        $result = false;
        // escape limit
        $submittedData['phoneEmailListFile'] = intval($submittedData['phoneEmailListFile']);
    ...
    public function saveAdditionalFields(
        ...
        $task->setPhoneEmailListFile($submittedData['phoneEmailListFile']);
    }
